...
print("Test 9: q bounces focus to the NvimTree sidebar when present")
vim.cmd("only")
local view_win = vim.api.nvim_get_current_win()
local edit_buf = vim.api.nvim_create_buf(true, false)
vim.api.nvim_win_set_buf(view_win, edit_buf)
vim.cmd("vsplit")
...
reset_capture()
viewer.open("/tmp/q.png")
assert_eq("  viewer opened in viewer window", vim.api.nvim_get_current_win(), view_win)
viewer.close("keymap_q_image")
assert_eq("  focus on NvimTree after q", vim.api.nvim_get_current_win(), tree_win)

print("Test 10: q with no sidebar leaves focus in the viewer window")
vim.cmd("only")
local solo_win = vim.api.nvim_get_current_win()
vim.api.nvim_win_set_buf(solo_win, vim.api.nvim_create_buf(true, false))
reset_capture()
viewer.open("/tmp/r.png")
viewer.close("keymap_q_image")
assert_eq("  focus unchanged with no sidebar", vim.api.nvim_get_current_win(), solo_win)

print("Test 11: video pauses on focus loss and resumes on focus gain")
vim.cmd("only")
fresh_buf()
reset_capture()
viewer.open("/tmp/clip.mp4")
local vbuf = vim.api.nvim_get_current_buf()
assert_true("  video viewer active", viewer.is_active())
reset_capture()
vim.api.nvim_exec_autocmds("WinLeave", { buffer = vbuf })
assert_true("  play_pause sent on focus loss", find_cmd("play_pause") ~= nil)
reset_capture()
vim.api.nvim_exec_autocmds("WinEnter", { buffer = vbuf })
assert_true("  play_pause sent on focus return", find_cmd("play_pause") ~= nil)
viewer.close()

print("Test 12: image does NOT pause on focus loss (no play_pause)")
vim.cmd("only")
fresh_buf()
reset_capture()
viewer.open("/tmp/still.png")
local ibuf = vim.api.nvim_get_current_buf()
reset_capture()
vim.api.nvim_exec_autocmds("WinLeave", { buffer = ibuf })
