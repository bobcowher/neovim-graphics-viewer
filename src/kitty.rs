...
impl KittyRenderer {
    pub fn new() -> Self {
        let writer: Box<dyn Write> = match std::env::var("NVIM_GFX_TTY") {
            Ok(path) => {
...
fn write_terminal_line(writer: &mut dyn Write, seq: &[u8]) -> Result<(), String> {
    writer.write_all(seq).map_err(|e| format!("write terminal: {e}"))?;
    writer.flush().map_err(|e| format!("flush terminal: {e}"))
}
...
fn write_kitty(
    ...
    write_terminal_line(writer, &buf)
}
